...
layui.use([ 'jquery', 'layer', 'form', 'table', 'laydate'], function() {
    ...
    laydate1.render(
        ...
        tableIns=table.render({
            ...
            ,url:'${pageContext.request.contextPath}/loginfo/loginfoList' //数据接口
        ...
        $("#doSearch").click(function(){
            ...
            tableIns.reload({
                url:"${pageContext.request.contextPath}/loginfo/loginfoList.action?"+params,
                page:{curr:1}
            })
        });
        ...
        table.on('tool(logInfoTable)', function(obj){
            ...
            if(layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.loginname + '】这条日志吗', function (index) {
                    //向服务端发送删除指令
                    $.post("${pageContext.request.contextPath}/loginfo/loginfoDelete.action", {loginid: data.loginid}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据 表格
                        tableIns.reload();
                    })
        ...
        function deleteBatch(id){
            ...
            $.each(data,function(i,item){
                if(i==0){
                    params+="ids="+item.loginid;
                }else{
                    params+="&ids="+item.loginid;
                }
                
            })
            layer.confirm('真的删除选择的这些日志吗', function(index){
                //向服务端发送删除指令
                $.post("${pageContext.request.contextPath}/loginfo/loginfoDeleteLot",params,function(res){
                    layer.msg(res.msg);
                    //刷新数据 表格
                    tableIns.reload();
                })
